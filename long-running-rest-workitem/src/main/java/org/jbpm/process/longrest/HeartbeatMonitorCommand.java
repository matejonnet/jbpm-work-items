...
public class HeartbeatMonitorCommand implements Command, Reoccurring {
    ...
    private Date nextScheduleTime = new Date();
    ...
    public ExecutionResults execute(CommandContext commandContext) throws Exception {
        ...
        logger.debug("Found {} active process instances.", processInstances.size());
        ...
        String interval = (String) commandContext.getData(Constant.HEARTBEAT_VALIDATION_VARIABLE);
        if (interval == null || interval.equals("")) {
            interval = "PT5S";
        }
        nextScheduleTime = Date.from(Instant.now().plus(Duration.parse(interval)));
